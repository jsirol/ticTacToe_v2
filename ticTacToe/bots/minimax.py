...
class AlphaBetaBot(gl.Player):
        ...
        if len(game_state.grid.possible_moves) > 0:
            ...
            if game_state.grid.dimension == 3:
                if game_state.turn == "X":
                    return self.alpha_beta(game_state, 5, True, self.simple_heuristic)
                else:
                    return self.alpha_beta(game_state, 5, False, self.simple_heuristic)
            else:
                # For bigger game use smaller depth and more complex heuristic.
                if game_state.turn == "X":
                    return self.alpha_beta(game_state, 0, True, self.heuristic_with_features)
                else:
                    return self.alpha_beta(game_state, 0, False, self.heuristic_with_features)
        else:
    ...
    def winning_features(self, game_state):
        ...
        x_improving = 0
        o_improving = 0
        for move in self.prune_possible_moves(game_state, stride=3):
    ...
    def heuristic_with_features(self, game_state):
        # Terminal positions
        if game_state.winner == "X":
            return MAX_REWARD
        elif game_state.winner == "O":
            ...
            return self.winning_features(game_state)
    ...
    def prune_possible_moves(self, game_state, stride=2):
    ...
    def alpha_beta(self, node, depth, maximizing_player, h):
        ...
        for move in self.prune_possible_moves(node):
            node.play_turn(move)
